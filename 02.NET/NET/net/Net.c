...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"cityid\":\"%d\",", device_state_init.expressage_city);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"area\":\"%d\",", device_state_init.expressage_area); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"id\":\"%d\",", device_state_init.expressage_id);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"state\":\"%d\"", device_state_init.expressage_state); 
	strcat(buf, text);
