...
mySta Update_oled_massage()
{
	...
	// 物料状态
	if(device_state_init.expressage_state == 3){
		sprintf(str, "状态: 出库      ");
	}else if(device_state_init.expressage_state == 2)
	{
		sprintf(str, "状态: 入库      ");
	}else if(device_state_init.expressage_state == 1)
	{
		sprintf(str, "状态: 未入库    ");
	}else if(device_state_init.expressage_state == 0)
	{
		sprintf(str, "状态: 正在查询   ");
	}
	
	OLED_ShowCH(0, 4, (unsigned char *)str);
	sprintf(str, "%02d S",device_state_init.transport_time);
...
mySta Update_device_massage()
{
	...
	// 判断入库
	if(device_state_init.expressage_state == 1 && device_state_init.transport_state == 1)
	{
		relay1out = 1; // 开始传输带
		device_state_init.transport_time = 20; 
	}
	// 监测到物体到达
	if(LEVEL1 == 0 && device_state_init.transport_state == 1){
		device_state_init.transport_state = 0;
		device_state_init.expressage_state = 2; //入库成功
		if (Data_init.App == 0) {
				Data_init.App = 1;
		}
		device_state_init.transport_time = 1; //延迟5S
	}
	// 关闭
	if(device_state_init.transport_time == 0 && device_state_init.transport_state == 0){
		relay1out = 0;
	}
	if (Data_init.App)
	{
...
void Automation_Close(void)
{
	...
	if( device_state_init.transport_state == 0 && device_state_init.transport_time== 0 
		&& device_state_init.expressage_state == 2 && device_state_init.expressage_app == 1){
		relay1out = 0;
		// 自动入库
		if(device_state_init.expressage_area == 1 ){
				area_1();
				clear();
		}else if(device_state_init.expressage_area == 2){
				area_2();
				clear();
		}else if(device_state_init.expressage_area == 3){
				area_3();
				clear();
		}
	}
...
void Check_Key_ON_OFF()
{
	...
	if (key == 0 && num_on > 0 && num_on < Key_Scan_Time) // 短按
	{
		...
		{
		case KEY1_PRES:
			if(device_state_init.expressage_state  == 1){
				device_state_init.expressage_state  = 1;
				// 开始传输
				device_state_init.transport_state  = 1;
